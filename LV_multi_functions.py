...
def find_NFD_computables(A, r=None, check = True):
...
def NFD_LV_multispecies(A, sub_equi, r=None, check=True, c_one=False):
    ...
    FD = 1 - np.einsum("nij,nij,nj->ni",
                       1 - FD_ij, sub_equi, 1/mono_equi)

    # compute invasion growth rates of each species
    r_i = r[...,np.newaxis] - np.einsum("nji,nki->njk", A, sub_equi)
    ...
    if check:
        ...
        try:
            ...
            if not (np.allclose([pars["ND"], pars["FD"]], [1 - NO[i], FD[i]],
                ...
                print("r_i automatic:", pars["r_i"])
                print("r_i LV_specific:", r_i[i])
                #print(NO_ij[i], "NO_ij\n")
                #print(A[i], "A\n")
                #print(c[i], "c\n")
